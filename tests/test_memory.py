...
@pytest.mark.asyncio
async def test_maybe_refresh_noop_below_interval(mock_provider: MockProvider, db_session: AsyncSession) -> None:
...
@pytest.mark.asyncio
async def test_maybe_refresh_creates_episode_summary(mock_provider: MockProvider, db_session: AsyncSession) -> None:
    settings = make_test_settings(memory_summary_interval=6)
    service = MemoryService(mock_provider, mock_provider, settings)
    ...
    summaries = (await db_session.scalars(select(EpisodeSummary).where(EpisodeSummary.session_id == session.id))).all()
...
@pytest.mark.asyncio
async def test_maybe_refresh_extracts_memory_facts(mock_provider: MockProvider, db_session: AsyncSession) -> None:
    settings = make_test_settings(memory_summary_interval=6)
    service = MemoryService(mock_provider, mock_provider, settings)
    session = SessionFactory(turn_count=6, last_summarized_turn=0)
    for i in range(1, 7):
    ...
    facts = (await db_session.scalars(select(MemoryFact).where(MemoryFact.session_id == session.id))).all()
    assert len(facts) == 2
...
@pytest.mark.asyncio
async def test_maybe_refresh_creates_relationships(mock_provider: MockProvider, db_session: AsyncSession) -> None:
    settings = make_test_settings(memory_summary_interval=6)
    service = MemoryService(mock_provider, mock_provider, settings)
    session = SessionFactory(turn_count=6, last_summarized_turn=0)
    for i in range(1, 7):
    ...
    rels = (await db_session.scalars(select(RelationshipState).where(RelationshipState.session_id == session.id))).all()
    assert len(rels) == 1
...
@pytest.mark.asyncio
async def test_memory_refresh_result_correct_counts(mock_provider: MockProvider, db_session: AsyncSession) -> None:
    settings = make_test_settings(memory_summary_interval=6)
    service = MemoryService(mock_provider, mock_provider, settings)
    session = SessionFactory(turn_count=6, last_summarized_turn=0)
    for i in range(1, 7):
