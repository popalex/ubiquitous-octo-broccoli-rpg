...
Exercised through the unified post-turn judge (world-only section) — the sole
production extraction path — so output nests under ``world_delta`` and the
...
CASES = [
    EvalCase(
        ...
        structural=[in_section("world_delta", entity_status("voss", "dead"))],
        max_tokens=1100,
    ),
    EvalCase(
        ...
        structural=[in_section("world_delta", has_inventory_change())],
        max_tokens=1100,
    ),
    EvalCase(
        ...
        structural=[in_section("world_delta", ledger_unchanged_after_apply(_LEDGER_WITH_DEAD_KAEL))],
        max_tokens=1100,
    ),
    EvalCase(
        ...
        structural=[in_section("world_delta", entity_not_resurrected("kael"))],
        max_tokens=1100,
    ),
]
