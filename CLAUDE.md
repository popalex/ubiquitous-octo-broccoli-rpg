...
- **Every metric instrument gets a dashboard panel.** When you add a `_meter.create_*` instrument in `app/telemetry.py`, add a matching panel to `observability/grafana/dashboards/rpg-metrics.json` in the same change — an un-dashboarded metric is invisible. Remember the OTel→Prometheus name mapping: dots become underscores, counters gain `_total`, histograms expose `_sum`/`_count`/`_bucket` (e.g. `rpg.post_turn.judge_calls` → `rpg_post_turn_judge_calls_total`).
